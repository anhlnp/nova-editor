...
var parseBasicAuthExpression = (expression) => {
...
var getBasicAuthCredentials = (authorization) => {
  const [scheme, encodedCredentials] = authorization?.split(/\s+/, 2) ?? [];
  const credentials = scheme?.toLowerCase() === "basic" && encodedCredentials !== void 0 ? decodeBase64(encodedCredentials) : "";
...
var matchWsAuthRoute = (route, pathname) => {
  ...
  const matchSegments = (routeIndex, pathnameIndex) => {
    ...
    return routeSegment === pathnameSegment && matchSegments(routeIndex + 1, pathnameIndex + 1);
  };
  return matchSegments(0, 0);
};
var findWsAuthRoute = (authRoutes, pathname) => authRoutes.find(({ route }) => matchWsAuthRoute(route, pathname));
var authenticateRequest = (request, authRoutes) => {
...
export {
  ...
  createWsAuthRouteFromPage,
  findWsAuthRoute,
  getBasicAuthCredentials,
  matchWsAuthRoute,
  parseBasicAuthExpression,
  parseWsAuth,
  parseWsAuthOrThrow,
  serializeWsAuth,
  validateBasicAuth,
